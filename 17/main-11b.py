...
def main():
    ...
    # Draw panels
    rmin = min(r for (r, c) in state.panels)
    rmax = max(r for (r, c) in state.panels)
    cmin = min(c for (r, c) in state.panels)
    cmax = max(c for (r, c) in state.panels)
    for r in range(rmin, rmax+1):
        for c in range(cmin, cmax+1):
            pixel = '#' if state.panels[r, c] == WHITE else ' '
            print(pixel, end='')
        print()
